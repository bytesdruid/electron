...
app_client1 = beaker.client.ApplicationClient(client=client, app=Electron.app, signer=acct_1.signer)
...
app_client2.call(Electron.add_electronics, pay_txn=pay_txn1, name="PhHONE1", description="PHONE1 DESCRIPTION", amount=2 * beaker.consts.algo, boxes=[(app_client1.app_id, _boxName)])

id = 1
_boxName = id.to_bytes(8, 'big')
txn = PaymentTxn(sender=acct_2.address, receiver=app_addr, amt=3000, sp=app_client2.get_suggested_params())
pay_txn1 = TransactionWithSigner(txn, acct_2.signer)
app_client2.call(Electron.add_electronics, pay_txn=pay_txn1, name="PhHONE2", description="PHONE2 DESCRIPTION", amount=3 * beaker.consts.algo, boxes=[(app_client1.app_id, _boxName)])
...
# # 5. BUY ELECTRONICS
txn = PaymentTxn(sender=acct_3.address, receiver=abi_decoded[0], amt=abi_decoded[3] *2, sp=app_client3.get_suggested_params())
...
# delete electronics at 0
# id = 0
# _boxName = id.to_bytes(8, 'big')
# elec = app_client2.call(Electron.delete_electronics, id=0, boxes=[(app_client1.app_id, _boxName)])
